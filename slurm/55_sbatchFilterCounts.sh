...
#SBATCH --output             filterClones_%j.out        # Standard output
#SBATCH --error              filterClones_%j.err        # Standard error


### SET I/O VARIABLES

IN=$data/mixcr/normalization/normalized_clones/             # Directory containing all input files. Should be one job per file
OUT=$data/normalization/filtered_clones/           # Directory where output files should be written
QC=$data/QC/std/
MYBIN=$tool/40_postProcess/filterClones.R          # Path to shell script or command-line executable that will be used
