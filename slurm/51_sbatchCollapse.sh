...
#SBATCH --output             collapseClones_%j.out        # Standard output
#SBATCH --error              collapseClones_%j.err        # Standard error


### SET I/O VARIABLES

IN=$data/normalization/normalized_clones/             # Directory containing all input files. Should be one job per file
OUT=$data/normalization/collapsed_clones/           # Directory where output files should be written
QC=$data/QC/
MYBIN=$tool/misc/combineDupCDR3.R          # Path to shell script or command-line executable that will be used
...
cmd="/home/exacloud/gscratch/CoussensLab/howellsf/R-4.0.2/bin/bin/Rscript $MYBIN -i $IN -o $OUT -q $QC" 
