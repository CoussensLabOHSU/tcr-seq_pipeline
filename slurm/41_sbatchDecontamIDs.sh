...
#SBATCH --output             decontamIDs_%j.out        # Standard output
#SBATCH --error              decontamIDs_%j.err        # Standard error


### SET I/O VARIABLES

CLONE=$data/mixcr/export_clones/             # Directory containing all input files. Should be one job per file
ALIGN=$data/mixcr/export_align/
OUT=$data/QC/std/econtam_id/           # Directory where output files should be written
MYBIN=$tool/40_postProcess/saveDecontamIDs.R          # Path to shell script or command-line executable that will be used
